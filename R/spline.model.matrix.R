spline.model.matrix <-
  function(object, data, ...){
    # spline model matrix for 'gammi' object
    # Nathaniel E. Helwig (helwig@umn.edu)
    # updated: 2024-07-18
    
    
    # This is essentially a reproduction of the rk.model.matrix() function 
    # in the "grpnet" R package (Helwig, 2024)
    ...
    for(j in 1:nvars){
      xlist[[j]] <- spline.basis(x = data[,variables[j]],
    ...
    for(k in 1:nterms){
      ...
      if(order[k] == 1L){
        ...
        for(j in 2:order[k]){
          xtemp <- rowKronecker(xtemp, xlist[[variables[tid[j]]]])
        }
  ...
  } # end spline.model.matrix


rowKronecker <- 
  function(X, Y){
    
    # This is a reproduction of the row.kronecker() function 
    # in the "grpnet" R package (Helwig, 2024)
